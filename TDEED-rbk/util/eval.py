...
from util.score import compute_amAP_mix
...
def mAPevaluate(model, dataset, classes, printed=True, event_team = False, metric = 'at1'):
    ...
    results = compute_amAP_mix(targets_numpy, detections_numpy, closests_numpy, classes, framerate=FPS_SN/dataset._stride, event_team = event_team)
    if printed:
...
def mAPevaluateTest(model, split, dataset, classes, printed=True, event_team = False, metric = 'at1', pred_file = None, postprocessing = 'SNMS'):
    ...
    #Get labels path
    labels_path = load_text(os.path.join('data', dataset._dataset, 'labels_path.txt'))[0]
    if dataset._dataset == 'soccernet':
        label_file = 'Labels-v2.json'
    else:
        label_file = 'Labels-ballvRBK.json'
    ...
    results = compute_amAP_mix(targets_numpy, detections_numpy, closests_numpy, classes, framerate=FPS_SN, event_team = event_team)
    ...
    if event_team:
        ...
        aux_classes = {}
        seen_labels = set()
        new_idx = 1  # Start from 1 to mimic the original dictionary style

        for k, v in sorted(classes.items(), key=lambda x: x[1]):
            base_label = k.split('-')[0]
            if base_label not in seen_labels:
                aux_classes[base_label] = new_idx
        ...
        results2 = compute_amAP_mix(targets_numpy, detections_numpy, closests_numpy, classes, framerate=FPS_SN, event_team = False)
...
def label2vector(labels, 
                 num_classes=17, 
                 framerate=2, 
                 EVENT_DICTIONARY={},
                 event_team=False):
    """
    Convert the JSON 'annotations' for a single half to a label matrix,
    but *only* for those events that are in EVENT_DICTIONARY. All events 
    not found in EVENT_DICTIONARY are skipped.
    """
    ...
    for annotation in labels["annotations"]:
        ...
        # e.g. 'Free kick'
        raw_event = annotation["label"]
        # e.g. 'left' or 'right' or some team name
        team_str = annotation.get('team', '')
        
        # Construct the event key, e.g. 'Free kick-left'
        # but only if event_team=True
        if event_team:
            event = raw_event + "-" + team_str
        else:
            event = raw_event
        
        # If this event is NOT in the dictionary, skip it
        if event not in EVENT_DICTIONARY:
            continue
        
        label_idx = EVENT_DICTIONARY[event] - 1
        
        half = int(time[0])  # '1' or '2', etc.
        ...
        # By default, set 1 or -1 for 'visibility'
        value = 1
        ...
        if half == 1:
            label_half1[frame][label_idx] = value
        # If your data might have “2nd half” annotations, you could store them differently.
        # Here only half == 1 is shown as your original code did.

    return label_half1

def predictions2vector(predictions,
                       num_classes=17,
                       framerate=2,
                       EVENT_DICTIONARY={},
                       event_team=False):
    """
    Convert the JSON 'predictions' from your model to a matrix, 
    but *only* for those events that are in EVENT_DICTIONARY. 
    Any other events are skipped.
    """
    vector_size = 120 * 60 * framerate
    ...
    for annotation in predictions["predictions"]:
        ...
        # Construct the event key if event_team=True
        if event_team:
            event = raw_event + "-" + team_str
        else:
            event = raw_event
        ...
        time_ms = annotation["position"]  # time in ms
        frame = int(framerate * (time_ms / 1000))
        frame = min(frame, vector_size - 1)

        value = annotation["confidence"]  # e.g. 0.85, etc.
        prediction_half1[frame][label_idx] = value
